...
bool w_logtest_check_input(char * input_json, cJSON ** req, OSList * list_msg) {
    ...
    *req = root;
    ...
    valid_json_string = cJSON_IsString(location) && location->valuestring != NULL && strlen(location->valuestring) > 0;
    ...
    valid_json_string = cJSON_IsString(event) && event->valuestring != NULL  && strlen(event->valuestring) > 0;
    valid_json_objet = cJSON_IsObject(event) && event->child != NULL;
    
    if ( !valid_json_objet && !valid_json_string) {
